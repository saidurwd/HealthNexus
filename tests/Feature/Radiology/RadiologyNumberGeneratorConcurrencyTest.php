...
use App\Services\Radiology\RadiologyNumberGenerator;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

/**
 * Mirrors LabNumberGeneratorConcurrencyTest's two-PDO-connection technique for
 * RadiologyNumberGenerator (used for radiology_orders.order_number and .accession_number).
 */
class RadiologyNumberGeneratorConcurrencyTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_generator_never_produces_duplicate_sequence_numbers_under_repeated_calls(): void
    {
        ...
        $generator = app(RadiologyNumberGenerator::class);
    ...
    public function test_row_lock_on_the_counter_blocks_a_concurrent_connection(): void
    {
        ...
        $pdoA->exec("INSERT INTO radiology_counters (company_id, branch_id, document_type, prefix, last_number, created_at, updated_at)
            VALUES ({$companyId}, NULL, 'ACC', 'RAD', 0, NOW(), NOW())");
        ...
        try {
            ...
            $pdoA->query("SELECT last_number FROM radiology_counters WHERE id = {$counterId} FOR UPDATE")->fetch();

            $blocked = false;

            try {
                $pdoB->beginTransaction();
                $pdoB->query("SELECT last_number FROM radiology_counters WHERE id = {$counterId} FOR UPDATE")->fetch();
                $pdoB->rollBack();
            } catch (\PDOException $e) {
            ...
            $pdoA->exec("UPDATE radiology_counters SET last_number = last_number + 1 WHERE id = {$counterId}");
            $pdoA->commit();

            $pdoB->beginTransaction();
            $lastNumber = $pdoB->query("SELECT last_number FROM radiology_counters WHERE id = {$counterId} FOR UPDATE")->fetchColumn();
            $pdoB->rollBack();
            ...
            $pdoA->exec("DELETE FROM radiology_counters WHERE id = {$counterId}");
        }
    }
}
