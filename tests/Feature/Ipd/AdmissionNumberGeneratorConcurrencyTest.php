...
use App\Services\Ipd\IpdNumberGenerator;

/**
 * Mirrors PharmacyNumberGeneratorConcurrencyTest's two-PDO-connection technique for
 * IpdNumberGenerator (used for ipd_admissions.admission_number).
 */
class AdmissionNumberGeneratorConcurrencyTest extends IpdTestCase
{
    public function test_generator_never_produces_duplicate_sequence_numbers_under_repeated_calls(): void
    {
        $company = Company::factory()->create();

        $generator = app(IpdNumberGenerator::class);

        $numbers = [];
        for ($i = 0; $i < 10; $i++) {
            $numbers[] = $generator->generateAdmissionNumber($company->id, null);
        }

        $this->assertCount(10, array_unique($numbers), 'Every generated admission number must be unique.');
    }
    ...
    public function test_row_lock_on_the_counter_blocks_a_concurrent_connection(): void
    {
        ...
        $pdoA->exec("INSERT INTO ipd_counters (company_id, branch_id, document_type, prefix, last_number, created_at, updated_at)
            VALUES ({$companyId}, NULL, 'ADM', 'ADC', 0, NOW(), NOW())");
        ...
        try {
            ...
            $pdoA->query("SELECT last_number FROM ipd_counters WHERE id = {$counterId} FOR UPDATE")->fetch();

            $blocked = false;

            try {
                $pdoB->beginTransaction();
                $pdoB->query("SELECT last_number FROM ipd_counters WHERE id = {$counterId} FOR UPDATE")->fetch();
                $pdoB->rollBack();
            } catch (\PDOException $e) {
            ...
            $pdoA->exec("UPDATE ipd_counters SET last_number = last_number + 1 WHERE id = {$counterId}");
            $pdoA->commit();

            $pdoB->beginTransaction();
            $lastNumber = $pdoB->query("SELECT last_number FROM ipd_counters WHERE id = {$counterId} FOR UPDATE")->fetchColumn();
            $pdoB->rollBack();
            ...
            $pdoA->exec("DELETE FROM ipd_counters WHERE id = {$counterId}");
        }
    }
}
